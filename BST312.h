...
void BST_312 <ItemType>::makeEmpty(TreeNode*& t) 
{
    ...
    if (t == NULL){
        ...
        makeEmpty(t->right); //traverse right part of tree
        makeEmpty(t->left); //traverse left part of tree
        delete (t); // delete dynamically allocated node
    } 
...
void BST_312 <ItemType>::insertItem(TreeNode*& t, const ItemType& newItem)
{
    ...
    //YOUR CODE GOES HERE
    if (t == NULL){
        TreeNode * temp = new TreeNode; //Dynamically allocate new node
        temp->data = newItem; //set data of new node
        temp->right = NULL; //set right pointer to NULL by default 
        temp->left = NULL; //set left pointer to NULL by default
        t = temp; //link old pointer to new node 
        return; 
    }
    else {
        if (newItem < t->data){
...
int BST_312 <ItemType>::countNodes()
{
    //YOUR CODE GOES HERE
    int numNodes = 1; 
    return (countNodes(root,numNodes)); 

}

template<class ItemType>
void BST_312 <ItemType>::preOrderTraversal(TreeNode* t,vector<ItemType>& result) const // node, left, right
{
    //YOUR CODE GOES HERE
    if (t!=NULL){ 
...
template<class ItemType>
void BST_312 <ItemType>::postOrderTraversal(TreeNode* t,vector<ItemType>& result) const //left, right, node
{
    //YOUR CODE GOES HERE
